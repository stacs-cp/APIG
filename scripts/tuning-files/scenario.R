...
repairConfiguration <- function(id, allConfigurations, parameters, digits, nConfsPreviousRaces=0){
    ...
    # read params.irace.meta for log-scale params, and update values in configuration
    correction <- list()
    metaFile <- './params.irace.meta'
    if (file.exists(metaFile)){
        lsLines <- readLines(con<-file(metaFile))
        close(con)
        for (s in lsLines){
            ls <- strsplit(s, ' ')[[1]]
            correction[[ls[1]]] <- as.integer(ls[2])
            configuration[[ls[1]]] <- correction[[ls[1]]]
        }
    }
    ...
    for (paramName in colnames(configuration)){
        if (!(paramName %in% c('.ID.','.PARENT.'))){
            ...
            if (paramName %in% names(correction))
                val <- val - correction[[paramName]]
    ...
    for (paramName in colnames(configuration)){
        if (!(paramName %in% c('.ID.','.PARENT.'))){
            ...
            if (paramName %in% names(correction))
                newVal <- newVal + correction[[paramName]]
