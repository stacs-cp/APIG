...
generateUniformConfiguration <- function(id, empty.configuration, parameters, namesParameters, repair, forbidden){ 
  finalConfiguration <- NULL
  forbidden.retries <- 1
  while (forbidden.retries < maxRetries) {
      ...
      if (!is.null(repair)) {
        configuration <- repair(configuration, parameters, digits, id)
      }
    ...
    if (forbidden.retries >= maxRetries)
      irace.error(paste("irace tried", maxRetries," times to sample from the model a configuration not forbidden without success, perhaps your constraints are too strict?"))
   return (finalConfiguration)
}

### Uniform sampling for the initial generation
sampleUniform <- function (parameters, nbConfigurations, digits, scenario,
                           forbidden = NULL, repair = NULL)
{
    ...
    nCores <- min(nbConfigurations, scenario$parallel)
    cat("\nSampling ", nbConfigurations, " configurations uniformly using ", nCores, " cores\n")
    configurations <- parallel::mclapply(c(1:nbConfigurations), 
                generateUniformConfiguration,
                empty.configuration=empty.configuration, parameters=parameters, namesParameters=namesParameters, repair=repair, forbidden=forbidden,
                mc.preschedule = !scenario$loadBalancing,
                mc.cores = nCores)
    configurations <- t(sapply(configurations,unlist))
    dimnames(configurations) <- list(NULL, c(namesParameters,".PARENT."))

    if (any(sapply(configurations, inherits, "try-error"))
        || any(sapply(configurations, is.null))) {
      ...
      cat(unique(unlist(
        configurations[sapply(
          configurations, inherits, "try-error")])), file = stderr())
  ...
  for (idxConfiguration in seq_len(nbNewConfigurations)) {
    ...
    while (forbidden.retries < 100) {
      ...
      if (is.null(forbidden)
        ...
        #nguyen: add this if clause over the line "newConfigurations[idxConfiguration..."to avoid duplicate candidates        
        if (nrow(merge(newConfigurations,configuration)) == 0){                    
    ...
    if (forbidden.retries >= maxRetries) {#nguyen: more retries for non-duplicated confs
