                           ...
                           forbidden = NULL, repair = NULL)
{
  ...
  if (!is.null(repair)){
    ...
    if (nCores>0){
      repairedConfigurations <- parallel::mclapply(c(1:nbConfigurations), repair, allConfigurations=newConfigurations, parameters=parameters, digits=digits, mc.preschedule = !scenario$loadBalancing, mc.cores = nCores)
    } else {
      repairedConfigurations <- lapply(c(1:nbConfigurations), repair, allConfigurations=newConfigurations, parameters=parameters, digits=digits) 
    }
    newConfigurations <- t(sapply(repairedConfigurations,unlist))
  }
...
sampleModel <- function (parameters, eliteConfigurations, model,
                         nbNewConfigurations, digits, scenario, startId,
                         forbidden = NULL,
                         repair = NULL)
{
  ...
  if (!is.null(repair)){
    nCores <- min(nbNewConfigurations, scenario$parallel)
    if (nCores>0){
      repairedConfigurations <- parallel::mclapply(c(startId:(startId+nbNewConfigurations-1)), repair, allConfigurations=newConfigurations, parameters=parameters, digits=digits, nConfsPreviousRaces=startId-1, mc.preschedule = !scenario$loadBalancing, mc.cores = nCores)
    } else {
      repairedConfigurations <- lapply(c(startId:(startId+nbNewConfigurations-1)), repair, allConfigurations=newConfigurations, parameters=parameters, nConfsPreviousRaces=startId-1, digits=digits) 
    }
    newConfigurations <- t(sapply(repairedConfigurations,unlist))
  }
